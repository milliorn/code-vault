...
data = pd.read_csv('jeopardy_starting\jeopardy.csv')
...
def filter_dataset(data, words):
    def filter(x): return all(word.lower() in x.lower() for word in words)
...
# For example, our function found a question that didn’t contain the word "king",
# however it did contain the word "viking" — it found the "king" inside "viking".
# Note that this also comes with some drawbacks —
# you would no longer find questions that contained words like "England's".
filtered = filter_dataset(data, ["King", "England"])
print(filter)
...
# https://discuss.codecademy.com/t/hurricane-analysis-challenge-project-python/462363
