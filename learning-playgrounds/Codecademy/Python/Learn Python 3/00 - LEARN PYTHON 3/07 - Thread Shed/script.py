...
# Now, iterate through daily_transactions
# (remember, this is a list of strings currently), and for each transaction,
# split the string around whatever character you replaced the ;,; artifacts with in Step 2.
...
# First, define an empty list transactions_clean.
transactions_clean = []
...
for x in daily_transactions_split:
    for y in x:
        transactions_clean.append(y.strip())
...
customers = [x for x in transactions_clean[::4]]
sales = [x for x in transactions_clean[1::4]]
thread_sold = [x for x in transactions_clean[2::4]]
...
# first split the string around the & character and then add each color indivudally to thread_sold_split.
for x in thread_sold:
    if '&' not in x:
        ...
        temp = x.split('&')
        for y in temp:
            thread_sold_split.append(y)

# Define a function called color_count that takes one argument, color.
# The function should iterate through thread_sold_split and count the number of times the item is equal to argument, color.
...
# Now, using the list colors, the string method .format(), and the function color_count,
# iterate through thread_sold_split and print a sentence that says how many threads of each color were sold today.
for x in colors:
    print("Thread Shed sold %s threads of %s thread today." %
          (color_count(x), x))
