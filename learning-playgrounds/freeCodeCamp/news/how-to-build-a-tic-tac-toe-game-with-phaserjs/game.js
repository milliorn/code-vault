...
(() => {
  ...
  function create() {
    ...
    statusText = scene.add
      .text(WIDTH / 2, BOARD + 12, "Player X's turn", {
        fontSize: "20px",
        color: "#111",
  ...
  function drawWinLine(res) {
    ...
    if (res.kind === "row") {
      ...
      overlayGfx.strokeLineShape(
        new Phaser.Geom.Line(pad, pad, BOARD - pad, BOARD - pad)
      );
    } else if (res.kind === "anti") {
      overlayGfx.strokeLineShape(
        new Phaser.Geom.Line(BOARD - pad, pad, pad, BOARD - pad)
      );
    }
  }
})();
