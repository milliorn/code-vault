#!/bin/bash
PSQL="psql -X --username=freecodecamp --dbname=periodic_table --tuples-only -c" # psql command to connect to the database

MAIN_PROGRAM() {
  if [[ -z $1 ]]; then # If no arguments are provided, print the help message
...
PRINT_ELEMENT() {                     # This function prints the element's information
  INPUT=$1                            # The element's name or atomic number
  if [[ ! $INPUT =~ ^[0-9]+$ ]]; then # If the input is not a number, then it's the element's name
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE symbol='$INPUT' OR name='$INPUT';") | sed 's/ //g')
  else # If the input is a number, then it's the element's atomic number
    ATOMIC_NUMBER=$(echo $($PSQL "SELECT atomic_number FROM elements WHERE atomic_number=$INPUT;") | sed 's/ //g')
  fi

  if [[ -z $ATOMIC_NUMBER ]]; then # If the atomic number is empty, then the element doesn't exist
    echo "I could not find that element in the database."
  else # If the atomic number is not empty, then the element exists
...
FIX_DB() { # This function fixes the database
...
START_PROGRAM() {
  CHECK=$($PSQL "SELECT COUNT(*) FROM elements WHERE atomic_number=1000;") # Check if the element with atomic number 1000 exists
  if [[ $CHECK -gt 0 ]]; then                                              # If it exists, delete it
    FIX_DB
    clear
  fi
  MAIN_PROGRAM $1
